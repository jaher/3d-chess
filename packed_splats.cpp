...
void parallel_radix_sort_descending_by_depth(
    ...
    // Extract original indices in their new order.
    {
        const size_t chunk = (n + num_threads - 1) / num_threads;
        std::vector<std::future<void>> jobs;
        jobs.reserve(num_threads);
...
void packed_splats_sort_and_upload(PackedSplats& ps,
    ...
    // The worker caches its scratch idx_/depth_ at the size of the
    // splat vector it was first started with. When the user swaps
    // tiers (e.g. 500k → full-res) the address &source stays the
    // same (it's a global vector reused for the new contents), but
    // the size changes — so we re-arm the worker whenever EITHER
    // the address OR the size differs from the last arm-up. Without
    // this, finished_idx_ is shorter than ps.count and the memcpy
    // below reads out of bounds.
    ...
    // Decide whether to queue a new sort. Always queue the first one;
    // afterwards only when the camera direction has changed by more
    // than `cos_threshold` from the LAST queued direction.
    ...
    // Pick up any completed ordering from the worker and upload.
