...
- **Chess.com puzzle of the day** — main-menu **Puzzles** button fetches the current Daily Puzzle from [api.chess.com](https://www.chess.com/news/view/published-data-api), sets up the position, and lets you play it out. The opponent prefers chess.com's canonical solution: the PGN body returned by the API is parsed into a UCI move sequence at load time, and as long as you follow the line, the AI replies are played directly from that sequence (no Stockfish call). The moment you diverge from the line, the AI falls back to a regular Stockfish bestmove for every subsequent reply. The puzzle resolves only when the game ends on the board (checkmate or stalemate), at which point a fresh random puzzle loads automatically. Desktop fetches via libcurl; the web build uses `fetch()` (no auth, no API key — public read-only data). On desktop the daily puzzle is also archived to `./puzzles/YYYY-MM-DD.md` (same format as `challenges/*.md`) the first time it's loaded each day, so the collection accumulates over time. To grow the archive without launching the app, install `tools/fetch_daily_puzzle.py` as a cron job:
  ```cron
