...
- **Chess.com puzzle of the day** — main-menu **Puzzles** button fetches the current Daily Puzzle from [api.chess.com](https://www.chess.com/news/view/published-data-api), sets up the position, and lets you play it out. Make any move you like; Stockfish always replies with the strongest move from the resulting position. The puzzle resolves when the position becomes decisive (checkmate, or a ≥ 5-pawn eval lead in your favour after the AI's reply), at which point a fresh random puzzle loads automatically. Desktop fetches via libcurl; the web build uses `fetch()` (no auth, no API key — public read-only data)
