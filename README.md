...
- **Mate-in-N challenge puzzles** loaded from `challenges/*.md`, with a glass-shatter transition between puzzles and a summary page at the end
...
contains the same pieces decimated to ~80,000 triangles each via
Blender's quadric collapse decimator — enough for the knight's mane
and crown finials to stay crisp under close zoom, while keeping the
total payload around ~24 MB (well under the 40 MB target for the
Pages deploy).
...
- **Lower-poly pieces**: the web build uses `models-web/` (~80K
  triangles per piece) instead of the desktop's `models/` (~1M
  triangles per piece). Differences are invisible at normal zoom
  and only become noticeable at extreme close-ups.
...
`shader`, `stl_model`, `linalg`, `chess_types`, `app_state`,
`game_state`, `challenge`, `cloth_flag`, `time_control`,
`ai_player`) that compiles for both targets. Only the thin platform
driver differs:
...
3d_chess/
  ...
  app_state.h/cpp          -- Shared UI state machine (modes, input, tick,
                              modals, clocks, physics) — platform-agnostic
  board_renderer.h/cpp     -- OpenGL rendering (PBR, shadows, overlays,
                              pregame dropdown, clock widget, flag draw)
  ...
  models-web/              -- Decimated STL piece models (web build, ~80k tris)
  tools/decimate_models.py -- Blender script that produces models-web/
  challenges/              -- Puzzle definition files
  tests/                   -- doctest-based unit tests (chess rules, FEN/UCI,
                              linalg). Run with `make test`
