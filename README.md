...
- **Find-all-forks / Pin-to-win** tactic puzzles enumerate every legal motif move; correct candidates accumulate, wrong (or repeated) attempts fire the same shake + Try-Again flow but **keep** the banked correct moves so the user only re-plays the latest line. The end-of-challenge summary lists every fork/pin found grouped by exercise number
