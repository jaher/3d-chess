...
- **Square-control heatmap** (press **C**): tints every square by which side attacks it more — blue (only White), red (only Black), purple (contested). A pure board-function teaching aid (`is_square_attacked` per square, no engine call), drawn under the pieces so it reads as board paint
