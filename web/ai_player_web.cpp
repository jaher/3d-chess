...
namespace web_ai {
    ...
    std::string eval_pv;        // full UCI principal variation of the best line
    int         eval_game_id = 0;
}
...
void on_eval_from_js(int cp, int idx, const char* best_uci, int game_id,
                     const char* second_uci, int second_cp, const char* pv) {
    web_ai::eval_cp = cp;
