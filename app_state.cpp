...
static void handle_board_click(AppState& a, double mx, double my,
    ...
    bool is_puzzle = (a.mode == MODE_PUZZLE);
    ...
    if (is_normal_game) {
        ...
        // Reject input while a fetch is in flight or the puzzle is
        // already over (just-solved → next puzzle is loading).
        if (a.puzzle_loading || a.puzzle_solved || a.puzzle_load_failed) return;
        if (gs.ai_thinking || gs.ai_animating || gs.game_over) return;
        // The user plays whichever side has the move per the FEN —
...
void app_enter_puzzle(AppState& a) {
    ...
    if (a.platform && a.platform->trigger_puzzle_fetch) {
        a.puzzle_loading = true;
        a.platform->trigger_puzzle_fetch(/*daily=*/true);
    } else {
        a.puzzle_loading = false;
        a.puzzle_load_failed = true;
...
void app_puzzle_ready(AppState& a, const char* json_body, bool daily) {
    a.puzzle_loading = false;
    if (a.mode != MODE_PUZZLE) return;  // user navigated away mid-fetch

    Puzzle p;
    if (!json_body || !*json_body || !puzzle_parse_json(json_body, p)) {
        a.puzzle_load_failed = true;
        set_status(a, daily
            ? "Couldn't load the Puzzle of the Day. Press ESC to go back."
            : "Couldn't load the next puzzle. Press ESC to go back.");
        queue_redraw(a);
        return;
    }
...
void app_release(AppState& a, double mx, double my, int width, int height) {
    ...
    switch (a.mode) {
        ...
        case MODE_PUZZLE:           release_puzzle(a, mx, my, width, height); return;
    }
}
...
void app_motion(AppState& a, double mx, double my, int width, int height) {
    switch (a.mode) {
        ...
        case MODE_PUZZLE:           motion_puzzle(a, mx, my, width, height); return;
    }
}
...
void app_scroll(AppState& a, double delta) {
    ...
    // While the D-key debug mode is on, lift the zoom-out cap so the
    // user can pull the camera outside the splat room (~55 units
    // wide) to position the light at a "window" angle. The
    // perspective frustum's far plane is 250, so 100 stays well
    // inside it. Normal play stays clamped at 40 so the chess scene
    // can't drift comically far.
    const float zoom_max = a.light_positioning ? 100.0f : 40.0f;
    if (a.zoom > zoom_max) a.zoom = zoom_max;
    queue_redraw(a);
}
...
void app_key(AppState& a, AppKey key) {
    ...
    if (key == KEY_D) {
        ...
        // Toggling OFF after the user zoomed past the normal-play
        // cap (40) would leave the chess scene tiny in the middle of
        // the splat room. Snap back so the next frame reads as the
...
static void start_ai_animation(AppState& a, int fc, int fr, int tc, int tr) {
    ...
    // sways, keycap press), so datacenter moves run slightly longer than
    // the medieval room's 0.5 s hop to give those a moment to read. This
    // started at 1.4 s, which showed the animation off nicely the first
    // few times and then just felt like waiting on the opponent. 0.6 s
    // keeps a touch of that character while staying responsive — the part
    // animations are normalised to this duration, so they simply play
    // proportionally faster rather than desyncing or clipping.
    gs.ai_anim_duration =
        (a.environment == AppState::Environment::DataCenter) ? 0.6f : 0.5f;
...
bool try_voice_command(AppState& a, const std::string& utterance) {
    ...
    case VoiceCommand::OpenChallenges:
        ...
        break;
    case VoiceCommand::OpenPuzzles:
        if (a.mode == MODE_MENU) app_enter_puzzle(a);
        break;
...
void apply_setting(AppState& a, const std::string& key, const std::string& val) {
    if (key == "splats_enabled") {
        ...
        // "datacenter" is the current token; "cable" is accepted for
        // backward-compat with settings.ini files written before the
        // cable-room → datacenter rename (enum value 1 is unchanged).
        if (val == "datacenter" || val == "cable")
...
std::string serialize_settings(const AppState& a) {
    ...
    const char* env_name =
        (a.environment == AppState::Environment::DataCenter) ? "datacenter" : "medieval";
    char buf[512];
    std::snprintf(buf, sizeof(buf),
